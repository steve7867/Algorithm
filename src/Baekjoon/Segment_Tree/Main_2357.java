...
class Main_2357 {
	...
	private static void processQuery() throws IOException {
		for (int i = 0; i < m; i++) {
			...
			.append(' ')
			.append(getMax(1, 1, n, a, b))
			.append('\n');
		}
		
	}

	private static int getMax(int node, int start, int end, int l, int r) {
		if (l > end || r < start)
			return Integer.MIN_VALUE;
		else if (l <= start && end <= r)
			return max[node];
		else
			return Math.max(getMax(node * 2, start, (start + end) / 2, l, r)
					, getMax(node * 2 + 1, (start + end) / 2 + 1, end, l, r));
	}
	...
	private static void makeTree() {
		...
		min = new int[treeSize + 1];
		max = new int[treeSize + 1];
	...
	private static int maxInit(int node, int start, int end) {
		if (start == end)
			...
			return max[node] = Math.max(maxInit(node * 2, start, (start + end) / 2)
	...
	private static int minInit(int node, int start, int end) {
		if (start == end)
			...
			return min[node] = Math.min(minInit(node * 2, start, (start + end) / 2)
