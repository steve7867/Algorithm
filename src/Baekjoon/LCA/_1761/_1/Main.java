...
public class Main {
    ...
    private static String getAns() throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            ...
            sb.append(getDistToLCA(a, b)).append('\n');
        }

        return sb.toString();
    }

    private static int getDistToLCA(int a, int b) {
        if (depth[a] < depth[b]) {
            int temp = a;
            a = b;
            b = temp;
    ...
    private static void makeTreeByDfs(int cur) {
        for (int i = 0; i < adjList[cur].size(); i++) {
            ...
            parent[next][0] = cur;
            dist[next][0] = distList[cur].get(i);
